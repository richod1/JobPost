...
export default function ImageUpload({name,icon,defaultValue=''}:{
    ...
    return(
        ...
        <div className="bg-gray-100 rounded-md size-24 inline-flex items-center content-center justify-center">
            ...
            {(!isUploading) && url &&(
                <Image src={url} alt={'uploaded image'} width={1024} height={1024} onLoadingComplete={()=>setIsImageLoading(false)} className="w-auto h-auto max-h-24"/>
            )}
            {!imgLoading && !url &&(
        ...
        <div className="mt-2">
            <input onChange={event=>upload(event)} ref={fileInRef} 
            type="file" 
            className="hidden"/>
